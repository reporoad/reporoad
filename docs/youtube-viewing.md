...
The normal website embeds whichever broadcast is currently live on the channel in the server runtime variable `YOUTUBE_CHANNEL_ID`, using YouTube's `embed/live_stream` player. A broadcaster restart that opens a new event therefore needs no website change. Live chat cannot be embedded per channel, so the Chat tab still addresses one video. Its ID is resolved from the channel's Atom feed (`/feeds/videos.xml?channel_id=…`): no API key, cloud project or quota is involved. The feed lists recent uploads newest first without marking which is live, so the newest entry is assumed to be the current broadcast — true for a channel that exists to stream, though publishing an ordinary video while live would point chat at that instead. `YOUTUBE_VIDEO_ID` (fallback `WuLbv_j9CGE`) is used while that lookup is unavailable. That is YouTube live chat, not the app's separate D1 chat room. The existing chat table/API is preserved without deletion but is not used by this interface.
...
Both iframes and watch links use one shared configuration fetched from `/api/broadcast-config` on page load and every 30 seconds. The lookup behind it is cached server-side for five minutes, and a failure is cached for one, so viewer polling never becomes one request to YouTube per viewer. A failed lookup keeps the configured fallback rather than taking the configuration down. Treat that fallback as load-bearing: if the feed changes shape or the newest entry is not the broadcast, chat quietly stays on `YOUTUBE_VIDEO_ID`. The channel's live page is deliberately not used — YouTube serves datacentre clients a variant whose canonical link is the literal string `undefined`, so it works from a laptop and fails from a Worker. Responses are not cached, unchanged IDs do not reload the iframe, and transient failures retain the last working ID. Invalid server values return a generic 503 without exposing the supplied value. Initial failures show a connecting message and retry. `embed_domain` uses the actual website hostname. The player sends its referrer with `strict-origin-when-cross-origin` rather than stripping it, as YouTube requires client identification. Desktop chat sign-in depends on YouTube cookies/browser settings; a link opens the same video on YouTube. Mobile visitors use that link because YouTube does not support mobile-web chat embeds.
...
This is not an automatic YouTube Go Live action. An offline channel can still display a replay or an unavailable player. Playback and chat availability must be checked in YouTube.
