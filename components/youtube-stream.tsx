...
export function YouTubeStream({ videoId }: { videoId: string | null }) {
  ...
  return <div className="youtube-view">
    ...
    <div className="youtube-view-note">
      ...
      <a href={watchUrl} target="_blank" rel="noopener noreferrer">Watch on YouTube <ExternalLink size={14}/></a>
    </div>
  </div>;
}
...
export function YouTubeChat({ online, videoId }: { online: number | null; videoId: string | null }) {
  ...
  return <section className="youtube-chat" aria-label="Shared YouTube chat">
    ...
    <div className="youtube-chat-fallback"><a className="btn" href={watchUrl} target="_blank" rel="noopener noreferrer">Open chat on YouTube <ExternalLink size={14}/></a><p>Use your YouTube account to post. If sign-in or chat is blocked here, open YouTube. Chat is available while the broadcast is live.</p></div>
  </section>;
}
