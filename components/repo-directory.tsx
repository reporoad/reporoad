...
export default function RepoDirectory({ repositories, status }: { repositories: Repository[]; status: string }) {
  ...
  const filtered = repositories.filter(r => `${r.fullName} ${r.language || ''}`.toLowerCase().includes(query.toLowerCase())).sort((a, b) => Number(b.placement === 'featured') - Number(a.placement === 'featured') || (descending ? b.stars - a.stars : a.stars - b.stars) || a.fullName.localeCompare(b.fullName));
