...
Submitting a URL does not establish ownership. In Add, connect GitHub and install the read-only RepoRoad GitHub App for your repository. Load the building, select Website-managed, and save. The server checks the authenticated GitHub identity and repository owner/admin permissions on every save. Website settings are stored in D1 against immutable repository and owner IDs; they override the optional file. Selecting Repository file / defaults removes the override. Transfers invalidate the old owner's override on the next metadata refresh. GitLab is not implemented yet.

GitHub authorization uses `https://reporoad.org/api/github/callback`, OAuth state and PKCE. Configure `GITHUB_CLIENT_SECRET` and a random `GITHUB_SESSION_SECRET` (at least 32 characters) as Sites secrets. Tokens are encrypted in D1, with one-hour sessions and Secure/HttpOnly/SameSite cookies. Expired sessions require reconnecting rather than retaining refresh tokens. Disconnect deletes the server session. The private key is not needed for this user-authorization flow. ChatGPT chat sign-in remains separate. The Sites alias sends Connect GitHub to the canonical reporoad.org domain so session cookies stay host-scoped.
