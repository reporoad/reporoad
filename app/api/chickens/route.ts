...
import { validChickenBatch, CHICKEN_UPSERT } from '@/lib/chickens';
import { readSchedule } from '@/lib/chicken-schedule';
...
export async function GET(request: Request) {
  try {
    ...
    const schedule = await readSchedule(now), { round } = schedule;
...
export async function POST(request: Request) {
  ...
  const now = Date.now();
  let round: number;
  try { round = (await readSchedule(now)).round; }
  catch { return Response.json({ error: 'Crossing unavailable.' }, { status: 503, headers: noStore }); }
  ...
  try {
    ...
    const results = await db.batch([
      ...
      db.prepare(CHICKEN_UPSERT).bind(value.id, round, value.total, key, nonce, round, now, value.total),
    ...
    if (!results[3].meta.changes) return Response.json({ error: 'This crossing has closed.' }, { status: 409, headers: noStore });
